...
public class Encrypt {
	...
	public static native byte[] encrypt(byte[] _buf);
	...
	public static void main(String[] args) throws Exception {
		...
		String pkg_name = map.get("-pkg");
		if (src_name == null || pkg_name == null) {
			System.out.println("usage: java Encrypt -src xxx.jar/war -pkg me/hebaceous");
			return;
		}

		Encrypt coder = new Encrypt();

		String suffix = src_name.substring(src_name.lastIndexOf('.'));
		String dst_name = map.get("-dst");
		if (dst_name == null || dst_name.equals(src_name))
			dst_name = src_name.substring(0, src_name.length() - 4) + "_encrypt"+suffix;

		System.out.printf("encode jar/war file: [%s ==> %s ]\n", src_name, dst_name);
		...
		for (Enumeration<JarEntry> enumeration = src_jar.entries(); enumeration.hasMoreElements();) {
			...
			if (name.contains(pkg_name) && name.endsWith(".class")) {
